return {
	...
	config = function()
		...
		local function set_fold_hl()
			local folded_icon_fg = vim.api.nvim_get_hl_by_name("Folded", true).foreground
			local folded_text_fg = vim.api.nvim_get_hl_by_name("Folded", true).foreground

			vim.api.nvim_set_hl(0, "FoldedIcon", { fg = folded_icon_fg })
			vim.api.nvim_set_hl(0, "FoldedText", { fg = folded_text_fg })
		end

		set_fold_hl()

		-- Update fold highlight colors on color scheme change
		...
		vim.api.nvim_set_keymap("n", "Q", "za", { noremap = true, silent = true })
